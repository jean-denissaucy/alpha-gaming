...
const Game = {
    ...
    async findAll() {
        const rows = await query(
            `SELECT g.id, g.categorie_id, g.titre_jeu AS game_name, g.lien AS link, g.image, g.note, c.name AS category_name
             FROM games g
    ...
    async findById(id) {
        const games = await query(
            `SELECT id, categorie_id, titre_jeu AS game_name, lien AS link, image, note
             FROM games
             WHERE id = ?
             LIMIT 1`,
            [id]
    ...
    async create({ categoryId, gameName, link, image, note }) {
        const result = await query(
            `INSERT INTO games (categorie_id, titre_jeu, lien, image, note)
             VALUES (?, ?, ?, ?, ?)`,
            [
                categoryId || null,
    ...
    async update(id, { categoryId, gameName, link, image, note }) {
        if (!id) return false;

        const result = await query(
            `UPDATE games
             SET categorie_id = ?, titre_jeu = ?, lien = ?, image = ?, note = ?
             WHERE id = ?`,
            [
                categoryId || null,
                String(gameName || '').trim(),
